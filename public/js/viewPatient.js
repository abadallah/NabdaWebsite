...
function addNewTScan(patientId,loginMail,InputImgScan){
    const spawn = require("child_process").spawn;
    const pythonProcess = spawn('python',["../../seg/MRI.py",InputImgScan ]);
    pythonProcess.stdout.on('data', (OutputScanName) => {
        alert(OutputScanName);

        var data = {"InputImgScan" : InputImgScan, "OutputScan" : OutputScanName };
        firebase.firestore().collection(`accounts/${loginMail}/users/${patientId}/patientData/Tscans`).add(data)
        .then(() => {
